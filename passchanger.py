...
automation_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'automation')
...
# PASSWORD GENERATOR
def generate_kaxz_password():
    """Generate toji password format (Fallback if needed)"""
    random_numbers = ''.join([str(random.randint(0, 9)) for _ in range(6)])
    return f"toji{random_numbers}"
...
def create_embed(title, description, color, fields=None):
    ...
    embed.set_footer(text="TOJI Password Changer Bot • High Security")
    return embed
                ...
                try:
                    async with aiohttp.ClientSession() as http_session:
                        ...
                        async with http_session.post(final_url, json=payload, headers=headers, timeout=5) as resp:
                            if resp.status == 200:
                                await channel.send(f"⚠️ {user.mention} failed CAPTCHA. Penalty: **80 Credits** applied via toji Mod API.")
                            else:
...
async def on_ready():
    print(f"\n╔{'═'*70}╗")
    print(f"║ WARDEN MS PASSWORD CHANGER BOT - ONLINE{' '*37}║")
    print(f"╚{'═'*70}╝")
    ...
    activity=discord.Activity(
        ...
        name="Changing Passwords | made by kaxz.gg2"
    )
)
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        title="<:Warden:1459595544241373369> Warden Password Changer Bot",
        description="**Full ACSR Automation** - WardenCloud Edition",
        color=COLOR_PRIMARY)
    ...
    embed.set_footer(text="WardenCloud • Full Automation System")
...
async def process_bulk(interaction: discord.Interaction,
    ...
    async def run_bulk():
        for ac in accounts:
            ...
            newpass = generate_kaxz_password()
...
# RENDER / 24-7 HOSTING
# Render Web Services expect the process to listen on PORT. This lightweight
# health server keeps the service reachable while the Discord bot runs.
...
if __name__ == "__main__":
    ...
    if not bot_token:
        raise RuntimeError("Set DISCORD_BOT_TOKEN in Render Environment Variables.")
